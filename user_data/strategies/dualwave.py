...
class dualwave(IStrategy):
    ...
    max_length = CategoricalParameter(
        [24, 48, 72, 96, 144, 192, 240], default=48, space="buy", optimize=False
    )
    ...
    tsl_target5 = DecimalParameter(
        low=0.25, high=0.4, decimals=1, default=0.3, space="sell", optimize=True, load=True
    )
    ts5 = DecimalParameter(
        low=0.04, high=0.06, default=0.05, space="sell", optimize=True, load=True
    )
    tsl_target4 = DecimalParameter(
        low=0.15, high=0.25, default=0.2, space="sell", optimize=True, load=True
    )
    ts4 = DecimalParameter(
        low=0.03, high=0.05, default=0.045, space="sell", optimize=True, load=True
    )
    tsl_target3 = DecimalParameter(
        low=0.10, high=0.15, default=0.15, space="sell", optimize=True, load=True
    )
    ts3 = DecimalParameter(
        low=0.025, high=0.04, default=0.035, space="sell", optimize=True, load=True
    )
    tsl_target2 = DecimalParameter(
        low=0.08, high=0.10, default=0.1, space="sell", optimize=True, load=True
    )
    ts2 = DecimalParameter(
        low=0.015, high=0.03, default=0.02, space="sell", optimize=True, load=True
    )
    tsl_target1 = DecimalParameter(
        low=0.06, high=0.08, default=0.06, space="sell", optimize=True, load=True
    )
    ts1 = DecimalParameter(
        low=0.01, high=0.016, default=0.013, space="sell", optimize=True, load=True
    )
    tsl_target0 = DecimalParameter(
        low=0.04, high=0.06, default=0.03, space="sell", optimize=True, load=True
    )
    ts0 = DecimalParameter(
        low=0.008, high=0.015, default=0.01, space="sell", optimize=True, load=True
    )
        ...
        for stop5 in self.tsl_target5.range:
            if current_profit > stop5:
                for stop5a in self.ts5.range:
                    self.dp.send_msg(
                        f"*** {pair} *** Profit: {current_profit} - lvl5 {stop5}/{stop5a} activated"
                    )
                    return stop5a
        for stop4 in self.tsl_target4.range:
            if current_profit > stop4:
                for stop4a in self.ts4.range:
                    self.dp.send_msg(
                        f"*** {pair} *** Profit {current_profit} - lvl4 {stop4}/{stop4a} activated"
                    )
                    return stop4a
        for stop3 in self.tsl_target3.range:
            if current_profit > stop3:
                for stop3a in self.ts3.range:
                    self.dp.send_msg(
                        f"*** {pair} *** Profit {current_profit} - lvl3 {stop3}/{stop3a} activated"
                    )
                    return stop3a
        for stop2 in self.tsl_target2.range:
            if current_profit > stop2:
                for stop2a in self.ts2.range:
                    self.dp.send_msg(
                        f"*** {pair} *** Profit {current_profit} - lvl2 {stop2}/{stop2a} activated"
                    )
                    return stop2a
        for stop1 in self.tsl_target1.range:
            if current_profit > stop1:
                for stop1a in self.ts1.range:
                    self.dp.send_msg(
                        f"*** {pair} *** Profit {current_profit} - lvl1 {stop1}/{stop1a} activated"
                    )
                    return stop1a
        for stop0 in self.tsl_target0.range:
            if current_profit > stop0:
                for stop0a in self.ts0.range:
                    self.dp.send_msg(
                        f"*** {pair} *** Profit {current_profit} - lvl0 {stop0}/{stop0a} activated"
                    )
                    return stop0a
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        if self.dp:
            ...
            informative["rsi_ma_pcnt"] = PC(
                informative, informative["rsi_ma"], informative["rsi_ma"].shift(1)
            )
            ...
            informative["t1_pc"] = PC(
                informative, informative["wave_t1"], informative["wave_t1"].shift(1)
            )

            # SMA
            informative["200_SMA"] = ta.SMA(informative["close"], timeperiod=200)
            informative["200_SMAPC"] = PC(
                informative, informative["200_SMA"], informative["200_SMA"].shift(1)
            )
            informative["from_200"] = ta.SMA(
                (
                    (((informative["close"] + informative["open"]) / 2) - informative["200_SMA"])
                    / informative["close"]
                )
                * 100,
                timeperiod=self.from2.value,
            )
            ...
            informative["rsi_weight"] = (
                (
                    informative["rsi_buy1"]
                    + informative["rsi_buy2"]
                    + informative["rsi_buy3"]
                    + informative["rsi_buy4"]
                )
                / 4
            ) * self.x01.value
            ...
            informative.loc[
                (
                    (informative["close"] > informative["200_SMA"])
                    & (informative["200_SMAPC"] > self.sma200_buy_pc.value)
                ),
                "sma_buy1",
            ] = 1
            informative.loc[
            ...
            informative["200SMA_weight"] = (
                (informative["sma_buy1"] + informative["sma_buy2"]) / 2
            ) * self.x02.value
            ...
            informative.loc[
                (informative["wave_t1"] > informative["wave_t1"].shift(1)), "wave_buy2"
            ] = 1
            informative.loc[
                (informative["wave_t1"] < informative["wave_t1"].shift(1)), "wave_buy2"
            ] = -1
            ...
            informative["wave_weight"] = (
                (informative["wave_buy1"] + informative["wave_buy2"] + informative["wave_buy3"]) / 3
            ) * self.x04.value

            informative["auto_buy"] = informative[
                ["rsi_weight", "wave_weight", "200SMA_weight", "from_weight"]
            ].sum(axis=1)
            ...
            informative.loc[
                (informative["rsi_ma_pcnt"] < self.rsi_ma_sellpc.value), "rsi_sell3"
            ] = 1
            informative.loc[
                (informative["rsi_ma_pcnt"] > self.rsi_ma_sellpc.value), "rsi_sell3"
            ] = -1
            ...
            informative["rsi_weight"] = (
                (
                    informative["rsi_sell1"]
                    + informative["rsi_sell2"]
                    + informative["rsi_sell3"]
                    + informative["rsi_sell4"]
                )
                / 4
            ) * self.y01.value
            ...
            informative.loc[
                (
                    (informative["close"] > informative["200_SMA"])
                    & (informative["200_SMAPC"] > self.sma200_sell_pc.value)
                ),
                "sma_sell1",
            ] = -1
            informative.loc[
            ...
            informative.loc[
                (informative["200_SMAPC"] > self.sma200_sell_pc.value), "sma_sell2"
            ] = -1
            informative.loc[(informative["200_SMAPC"] < self.sma200_sell_pc.value), "sma_sell2"] = 1

            informative["200SMA_weight"] = (
                (informative["sma_sell1"] + informative["sma_sell2"]) / 2
            ) * self.y02.value
            ...
            informative.loc[
                (informative["wave_t1"] > informative["wave_t1"].shift(1)), "wave_sell2"
            ] = -1
            informative.loc[
                (informative["wave_t1"] < informative["wave_t1"].shift(1)), "wave_sell2"
            ] = 1
            ...
            informative["wave_weight"] = (
                (informative["wave_sell1"] + informative["wave_sell2"] + informative["wave_sell3"])
                / 3
            ) * self.y04.value

            informative["auto_sell"] = informative[
                ["rsi_weight", "wave_weight", "200SMA_weight", "from_weight_sell"]
            ].sum(axis=1)
        ...
        for length in self.filterlength.range:
            ...
            dataframe[f"ema_dif{length}"] = (
                dataframe[f"ema_1{length}"] - dataframe[f"ema_2{length}"]
            )
            dataframe[f"zema_{length}"] = (
                dataframe[f"ema_1{length}"] + dataframe[f"ema_dif{length}"]
            )
        ...
        dataframe["from_200"] = ta.SMA(
            (
                (((dataframe["close"] + dataframe["open"]) / 2) - dataframe["200_SMA"])
                / dataframe["close"]
            )
            * 100,
            timeperiod=self.from15.value,
        )
        ...
        dataframe.loc[
            (
                ...
                & (dataframe["200_SMAPC"] > self.sma200_buy_pc.value)
            ),
            "sma_buy1",
        ] = 1
        dataframe.loc[
        ...
        dataframe["200SMA_weight"] = (
            (dataframe["sma_buy1"] + dataframe["sma_buy2"]) / 2
        ) * self.x06.value
        ...
        dataframe["willr_weight"] = (
            (dataframe["willr_buy1"] + dataframe["willr_buy2"] + dataframe["willr_buy3"]) / 3
        ) * self.x07.value
        ...
        dataframe.loc[
            (
                (dataframe["wave_t1"] > dataframe["wave_t1_2h"])
                & (dataframe["wave_t1_2h"] < self.slow_wave_buy.value)
            ),
            "wave_buy4",
        ] = 1
        dataframe.loc[
        ...
        dataframe["auto_buy"] = dataframe[
            ["rsi_weight", "willr_weight", "200SMA_weight", "from_weight", "wave_weight"]
        ].sum(axis=1)
        ...
        dataframe.loc[
            (
                (dataframe["close"] > dataframe["200_SMA"])
                & (dataframe["200_SMAPC"] > self.sma200_sell_pc.value)
            ),
            "sma_sell1",
        ] = 1
        dataframe.loc[
        ...
        dataframe["200SMA_weight_sell"] = (
            (dataframe["sma_sell1"] + dataframe["sma_sell2"]) / 2
        ) * self.y06.value
        ...
        dataframe["willr_weight_sell"] = (
            (dataframe["willr_sell1"] + dataframe["willr_sell2"] + dataframe["willr_sell3"]) / 3
        ) * self.y07.value
        ...
        dataframe.loc[
            (
                (dataframe["wave_t1"] > dataframe["wave_t1_2h"])
                & (dataframe["wave_t1_2h"] > self.slow_wave_sell.value)
            ),
            "wave_sell4",
        ] = 1
        dataframe.loc[
        ...
        dataframe["wave_weight"] = (
            (
                dataframe["wave_sell1"]
                + dataframe["wave_sell2"]
                + dataframe["wave_sell3"]
                + dataframe["wave_sell4"]
            )
            / 4
        ) * self.y09.value

        dataframe["auto_sell"] = dataframe[
            ["rsi_weight_sell", "willr_weight_sell", "200SMA_weight_sell", "from_weight_sell"]
        ].sum(axis=1)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        ] = (1, "auto buy bullzzz up")

        dataframe.loc[
            (
                (dataframe["auto_buy_decision"] >= (self.auto_buy.value + self.auto_buy_down.value))
                & (
                    dataframe["auto_buy_decision_2h"]
                    >= (self.auto_buy_2.value + self.auto_buy_down_2.value)
                )
                & (dataframe["auto_buy_decision"] >= dataframe["auto_buy_decision"].shift(1))
                & (dataframe["200_SMA_2h"] > dataframe["200_SMA_2h"].shift(8))
                & (dataframe["200_SMA"] < dataframe["200_SMA"].shift(1))
            ...
            ["enter_long", "enter_tag"],
        ] = (1, "auto buy bullzzz down")

        dataframe.loc[
            (
                (
                    dataframe["auto_buy_decision"]
                    >= (self.auto_buy.value + self.auto_buy_bearzzz.value)
                )
                & (
                    dataframe["auto_buy_decision_2h"]
                    >= (self.auto_buy_2.value + self.auto_buy_bearzzz_2.value)
                )
                & (dataframe["auto_buy_decision"] >= dataframe["auto_buy_decision"].shift(1))
                & (dataframe["200_SMA_2h"] < dataframe["200_SMA_2h"].shift(8))
                & (dataframe["200_SMA"] > dataframe["200_SMA"].shift(1))
        ...
        dataframe.loc[
            (
                (
                    ...
                    >= (
                        self.auto_buy.value
                        + self.auto_buy_bearzzz.value
                        + self.auto_buy_bearzzz_down.value
                    )
                )
                & (
                    dataframe["auto_buy_decision_2h"]
                    >= (self.auto_buy_2.value + self.auto_buy_bearzzz_down_2.value)
                )
                & (dataframe["auto_buy_decision"] >= dataframe["auto_buy_decision"].shift(1))
                & (dataframe["200_SMA_2h"] < dataframe["200_SMA_2h"].shift(8))
                & (dataframe["200_SMA"] < dataframe["200_SMA"].shift(1))
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                (
                    dataframe["auto_sell_decision"]
                    >= (self.auto_sell_bull.value + self.auto_sell_bear.value)
                )
                & (dataframe["200_SMA_2h"] > dataframe["200_SMA_2h"].shift(8))
                & (dataframe["volume"] > 0)
            ),
