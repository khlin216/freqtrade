...
class x_zvwap06(IStrategy):
    ...
    pHSL = DecimalParameter(
        -0.500, -0.040, default=-0.08, decimals=3, space="sell", optimize=False, load=True
    )
        ...
        **kwargs,
        ...
        if last_candle is not None:
            ...
            if (current_profit > 0.05) & (last_candle["rsi"] < 42.0) & (last_candle["cmf"] < -0.4):
                ...
                (current_profit > 0.05)
                & (last_candle["rsi"] < 43.0)
                & (last_candle["cmf"] < -0.0)
                & (last_candle["cmf_15m"] < -0.0)
                & (last_candle["cmf_1h"] < -0.0)
            ):
                return f"profit exit2( {buy_tag})"
            elif (
                (current_profit > 0.05)
                & (last_candle["rsi"] < 41.0)
                & (last_candle["cmf"] < -0.2)
                & (last_candle["cmf_1h"] < -0.0)
            ):
                ...
                (current_profit > 0.05)
                & (last_candle["rsi"] < 44.0)
                & (last_candle["cmf"] < -0.1)
                & (last_candle["cmf_15m"] < -0.1)
                & (last_candle["cmf_1h"] < -0.1)
            ):
                return f"profit exit4( {buy_tag})"
            elif (
                (current_profit > 0.05)
                & (last_candle["rsi"] < 40.0)
                & (last_candle["cmf"] < -0.2)
                & (last_candle["cmf_15m"] < -0.2)
            ):
                ...
                (current_profit > 0.02) & (last_candle["rsi"] < 30.0) & (last_candle["cmf"] < -0.4)
            ):
                return f"profit exit bear1( {buy_tag})"
            elif (
                (current_profit > 0.03) & (last_candle["rsi"] < 35.0) & (last_candle["cmf"] < -0.4)
            ):
                return f"profit exit bear2( {buy_tag})"
            elif (
                (current_profit > 0.01) & (last_candle["rsi"] < 25.0) & (last_candle["cmf"] < -0.6)
            ):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe[f"range{self.range_filter_length.value}"] = range_filter(
            dataframe=dataframe, length=self.range_filter_length.value
        )
        ...
        dataframe["rsi_fast_avg"] = (
            dataframe["rsi_fast"] + dataframe["rsi_fast1"] + dataframe["rsi_fast2"]
        ) / 3
        ...
        informative["live_data_ok"] = (
            dataframe["volume"].rolling(window=48, min_periods=48).min() > 0
        )
        ...
        dataframe["long_term_price_drop"] = np.where(
            (
                # 4% drop in 20m
                (
                    dataframe["close"].rolling(4).max() * 0.96
                    > dataframe["close"].rolling(6).mean()  # 0 buys 20220101
                )
                |
                # 4% drop in 25m
                (
                    dataframe["close"].rolling(6).max() * 0.96
                    > dataframe["close"].rolling(6).mean()  # 0 buys 20220101
                )
                |
                # 5% drop in 30m
                (
                    dataframe["close"].rolling(6).max() * 0.95
                    > dataframe["close"].rolling(6).mean()  # 0 buys 20220101
                )
                |
                # 5% drop in 40m
                (
                    dataframe["close"].rolling(8).max() * 0.95
                    > dataframe["close"].rolling(6).mean()  # 0 buys 20220101
                )
                |
                # 6% drop in 40m
        ...
        dataframe["deadfish"] = np.where(
            ...
            & (
                dataframe["close"]
                > dataframe["bb_middleband2"] * self.sell_deadfish_bb_factor.value
            )
            & (dataframe["cmf"] < 0.0)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                ...
                & (dataframe["tcp_percent_4"] > self.tcp_percent_4_value.value)
                &  # 0.053)
                # (dataframe['cti'] < -0.8) & # -0.8)
                (dataframe["cti"] < self.cti.value)
                &  # zobrate z bbrtr -0.087
                # (dataframe['rsi'] < 35) &
        ...
        if count_of_buys > 0:
            ...
            if last_order.order_filled_date is not None:
                ...
                if time_difference.total_seconds() < 300:
                    logger.info(
                        f"Skipping SO buy for {trade.pair} because 300 sec have not passed since the last filled buy order."
                    )
                    return None

        if last_candle["pump_strength"] < -0.15:
            logger.info(
                f"DCA for {trade.pair} waiting for pump_strength ({last_candle['pump_strength']}) to rise above -0.15"
            )
            return None

        if count_of_buys > 1 and last_candle["rsi_fast_avg"] < 20:
            logger.info(
                f"DCA for {trade.pair} waiting for rsi_fast ({last_candle['rsi_fast_avg']}) to rise above 20"
            )
            return

        if count_of_buys > 2 and last_candle["smi_trend_15m"] < -1:
            logger.info(
                f"DCA for {trade.pair} waiting for smi_trend 1+ actual trend ({last_candle['smi_trend_15m']})"
            )
            return None
