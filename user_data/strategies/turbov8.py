...
class turbov8(IStrategy):
    ...
    dca1 = DecimalParameter(
        low=0.01, high=0.03, decimals=2, default=0.02, space="buy", optimize=True, load=True
    )
    dca2 = DecimalParameter(
        low=0.03, high=0.05, decimals=2, default=0.04, space="buy", optimize=True, load=True
    )
    dca3 = DecimalParameter(
        low=0.05, high=0.07, decimals=2, default=0.06, space="buy", optimize=True, load=True
    )

    # trailing stop loss optimiziation
    tsl_target5 = DecimalParameter(
        low=0.3, high=0.4, decimals=1, default=0.3, space="sell", optimize=True, load=True
    )
    ts5 = DecimalParameter(
        low=0.04, high=0.06, default=0.05, space="sell", optimize=True, load=True
    )
    tsl_target4 = DecimalParameter(
        low=0.18, high=0.3, default=0.2, space="sell", optimize=True, load=True
    )
    ts4 = DecimalParameter(
        low=0.03, high=0.05, default=0.045, space="sell", optimize=True, load=True
    )
    tsl_target3 = DecimalParameter(
        low=0.12, high=0.18, default=0.15, space="sell", optimize=True, load=True
    )
    ts3 = DecimalParameter(
        low=0.025, high=0.04, default=0.035, space="sell", optimize=True, load=True
    )
    tsl_target2 = DecimalParameter(
        low=0.07, high=0.12, default=0.1, space="sell", optimize=True, load=True
    )
    ts2 = DecimalParameter(
        low=0.015, high=0.03, default=0.02, space="sell", optimize=True, load=True
    )
    tsl_target1 = DecimalParameter(
        low=0.04, high=0.07, default=0.06, space="sell", optimize=True, load=True
    )
    ts1 = DecimalParameter(
        low=0.01, high=0.016, default=0.013, space="sell", optimize=True, load=True
    )
    tsl_target0 = DecimalParameter(
        low=0.02, high=0.05, default=0.03, space="sell", optimize=True, load=True
    )
    ts0 = DecimalParameter(
        low=0.008, high=0.015, default=0.013, space="sell", optimize=True, load=True
    )
        ...
        for stop5 in self.tsl_target5.range:
            if current_profit > stop5:
                for stop5a in self.ts5.range:
                    self.dp.send_msg(
                        f"*** FEEDBACK *** Pair {pair} profit {current_profit} - lvl5 {stop5a} stoploss activated"
                    )
                    return stop5a
        for stop4 in self.tsl_target4.range:
            if current_profit > stop4:
                for stop4a in self.ts4.range:
                    self.dp.send_msg(
                        f"*** FEEDBACK *** Pair {pair} profit {current_profit} - lvl4 {stop4a} stoploss activated"
                    )
                    return stop4a
        for stop3 in self.tsl_target3.range:
            if current_profit > stop3:
                for stop3a in self.ts3.range:
                    self.dp.send_msg(
                        f"*** FEEDBACK *** Pair {pair} profit {current_profit} - lvl3 {stop3a} stoploss activated"
                    )
                    return stop3a
        for stop2 in self.tsl_target2.range:
            if current_profit > stop2:
                for stop2a in self.ts2.range:
                    self.dp.send_msg(
                        f"*** FEEDBACK *** Pair {pair} profit {current_profit} - lvl2 {stop2a} stoploss activated"
                    )
                    return stop2a
        for stop1 in self.tsl_target1.range:
            if current_profit > stop1:
                for stop1a in self.ts1.range:
                    self.dp.send_msg(
                        f"*** FEEDBACK *** Pair {pair} profit {current_profit} - lvl1 {stop1a} stoploss activated"
                    )
                    return stop1a
        for stop0 in self.tsl_target0.range:
            if current_profit > stop0:
                for stop0a in self.ts0.range:
                    self.dp.send_msg(
                        f"*** FEEDBACK *** Pair {pair} profit {current_profit} - lvl0 {stop0a} stoploss activated"
                    )
                    return stop0a
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if informative[f"sma_{fast}"].iloc[-1] > informative[f"sma_{slow}"].iloc[-1]:
            ...
            if self.run_from_bear == 1:
                self.dp.send_msg(
                    f"MARKET STATUS: Bear is gone! Time to wake up from hibernations...",
                    always_send=True,
                )
                self.run_from_bear = 0
        elif (
            informative[f"sma_{fast}"].iloc[-1] < informative[f"sma_{slow}"].iloc[-1]
            and informative[f"sma_{fast}"].iloc[-1] < informative[f"sma_{fast}"].iloc[-2]
        ).all():
            dataframe["Run_From_Bear"] = 1
            if self.run_from_bear == 0:
                self.dp.send_msg(
                    f"MARKET STATUS: Bear sighted! Selling off and going into hibernation...",
                    always_send=True,
                )
            ...
            dataframe["Run_From_Bear"] = -1
            self.dp.send_msg(
                f"MARKET STATUS: Bear Lurking! Grab the Lube, This could hurt...", always_send=True
            )

        if (
            dataframe["30_SMA"].iloc[-1] > dataframe["200_SMA"].iloc[-1]
            and dataframe["30_SMA"].iloc[-1] > dataframe["30_SMA"].iloc[-2]
            and dataframe["200_SMA"].iloc[-1] > dataframe["200_SMA"].iloc[-2]
        ).all():
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            ((dataframe["Run_From_Bear"] == 1) & (dataframe["volume"] > 0)),
            ["exit_long", "exit_tag"],
        ] = (1, "fucking bearzzz")
